...
def record_agent(id, is_group, name):
    mycursor.execute(SQL_QUERY_3, (id, is_group, name))
    mydb.commit()
...
def record_message(chat_id, message_id, reply_chat_id, reply_message_id, timestamp):
...
def record_update(update_id, update_json):
    mycursor.execute("insert into updates values (%s, %s)", (update_id, update_json))
    mydb.commit()
